...
export class ProduitComponent {
  ...
  selectedFile: File | null = null;        // <- Pour l'image
  imagePreview: string | ArrayBuffer | null = null; // <- Pour l’aperçu

  baseImagePath: string = 'C:/Users/Downlowds/'; // Mets ton chemin local ici
    ...
    if (this.editProduit && this.editProduit.id) {
      this.produitService.updateProduit(this.editProduit.id, this.editProduit).subscribe(
        (response) => {
          ...
          this.getAllProduits(); // Mettre à jour la liste des produits après mise à jour
  ...
  setEditProduit(produit: Produit) {
    this.editProduit = { ...produit }; // Cloner l'objet pour l'édition (évite la modification directe de l'objet original)
  }
  
  cancelEdit() {
    this.editProduit = null; // Annuler l'édition (réinitialiser l'édition)
  }
  ...
  onImageSelected(event: any): void {
    ...
    if (file) {
      ...
      const fullPath = this.baseImagePath + fileName;
  
      if (this.editProduit) {
        this.editProduit.imageUrl = fullPath;
      } else {
        this.produit.imageUrl = fullPath;
      }
  
      // Optionnel pour aperçu
